...
class adrv9009_dual(rx_tx, context_manager, sync_start):
    ...
    def __init__(self, uri="", dev_suffix = "-1", jesd_monitor=False, jesd=None):
        """ boyang NOTE: dev_suffix can only by -1 or -2 for two 9009 attached, used to find_device call below,
            This is HARD-CODED in device tree"""

        if dev_suffix not in ("-1", "-2"):
            raise ValueError(f"dev_suffix for adrv9009_dual must be string and either -1 or -2")

        context_manager.__init__(self, uri, self._device_name)

        self._ctrl = self._ctx.find_device(f"adrv9009-phy{dev_suffix}")
        self._rxadc = self._ctx.find_device(f"axi-adrv9009-rx-hpc{dev_suffix}")
        self._rxobs = self._ctx.find_device(f"axi-adrv9009-rx-obs-hpc{dev_suffix}")
        self._txdac = self._ctx.find_device(f"axi-adrv9009-tx-hpc{dev_suffix}")
